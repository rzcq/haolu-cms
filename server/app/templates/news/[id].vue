        ...
        <nav class="flex text-sm" aria-label="Breadcrumb">
          <ol class="flex items-center space-x-2">
            ...
            <li class="flex items-center">
              ...
              <span class="ml-2 text-primary">{{ newsDetail?.title?.[locale] ?? newsDetail?.title?.en }}</span>
            </li>
          </ol>
        </nav>
      </div>
            ...
            <div class="flex items-center justify-center text-gray-500 text-sm space-x-4">
              <span>
                {{ t('news.detail.category') }}：{{
                  newsDetail.category.categoryName?.[locale] ?? newsDetail?.category.categoryName?.en
                }}
              </span>
...
<script setup lang="ts">
  ...
  const summary = computed(() => newsDetail.value.summary?.[locale.value] || newsDetail.value?.summary?.en);
  const keywords = computed(() => newsDetail.value.keywords?.[locale.value] || newsDetail.value?.keywords?.en);
