...
<script setup lang="ts">
  ...
  const templateMap: Record<string, () => Promise<any>> = {
    ...
    contact: () => import('../templates/contact/index.vue')
  };
  ...
  const findCategoryByPath = async (pathSegments: string[]): Promise<any> => {
    ...
    if (segmentsForLookup.length === 1) {
      const result: any = await $fetch('/api/public/cms/category/by-urlkey/' + segmentsForLookup[0]);
      return result?.data || null;
    }

    // 多级栏目：逐级匹配
    const result: any = await $fetch('/api/public/cms/category/by-path', {
      method: 'POST',
      body: { pathSegments: segmentsForLookup }
    });
    return result?.data || null;
  };
