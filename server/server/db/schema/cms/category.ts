...
export const categoryTable = mysqlTable('category', {
  ...
  // 分类名称（多语言）
  categoryName: json('category_name').$type<Record<string, string>>().default({ zh: '', en: '' }),
  // 分类描述（多语言）
  description: json('description').$type<Record<string, string>>().default({ zh: '', en: '' }),
  // 关键词（多语言，用于SEO）
  keywords: json('keywords').$type<Record<string, string>>().default({ zh: '', en: '' }),
  // 分类类型：1=新闻分类, 2=产品分类
  ...
  // 产品参数模板（JSON，仅产品分类的顶级分类可配置）
  paramTemplate: json('param_template').$type<ParamField[]>().default([]),
  // 状态：0=草稿, 1=发布, 2=下架
