  ...
  async list(params: Partial<Product> & queryParams) {
    ...
    const [rows, total] = await Promise.all([
      db
        .select()
        .from(productTable)
        .leftJoin(categoryTable, eq(productTable.categoryId, categoryTable.categoryId))
        .where(where)
        .limit(limit)
        .offset(offset)
        .orderBy(asc(productTable.orderNum), desc(productTable.createTime)),
      db.$count(productTable, where)
    ]);

    // 将 leftJoin 结果转换为 { ...product, category } 格式
    ...
    const [rows, total] = await Promise.all([
      db
        .select({
          ...
          mainImage: productTable.mainImage,
          viewCount: productTable.viewCount,
          isNew: productTable.isNew,
          isRecommend: productTable.isRecommend,
          orderNum: productTable.orderNum,
          categoryId: productTable.categoryId,
          category: {
            categoryId: categoryTable.categoryId,
            categoryName: categoryTable.categoryName,
            mpath: categoryTable.mpath
          }
        })
        .from(productTable)
        .leftJoin(categoryTable, eq(productTable.categoryId, categoryTable.categoryId))
