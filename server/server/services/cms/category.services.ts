...
import { Category, categoryTable, NewCategory, ParamField } from '~~/server/db/schema/cms/category';
...
export class CategoryServices {
  ...
  async list(params: Partial<Category> & { pageNum?: number; limit?: number }, filterParams?: any) {
    const pageNum = Number(params.pageNum) || 1; const limit = Number(params.limit) || 10;
  ...
  /**
   * 获取分类的参数模板（含继承逻辑）
   * 优先使用自身模板，为空则沿 mpath 向上查找父级模板
   */
  async getParamTemplate(categoryId: number): Promise<ParamField[]> {
    const category = await this.findById(categoryId);
    if (!category) return [];

    // 如果当前分类有模板配置，直接返回
    if (category.paramTemplate && category.paramTemplate.length > 0) {
      return category.paramTemplate;
    }
    ...
    if (category.mpath) {
      ...
      for (const ancestorId of ancestorIds) {
        ...
        if (ancestor?.paramTemplate && ancestor.paramTemplate.length > 0) {
          return ancestor.paramTemplate;
        }
      }
    }
