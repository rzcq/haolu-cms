  ...
  async list(params: Partial<News> & queryParams) {
    ...
    const [rows, total] = await Promise.all([
      db
        .select()
        .from(newsTable)
        .leftJoin(categoryTable, eq(newsTable.categoryId, categoryTable.categoryId))
        .where(where)
        .limit(limit)
        .offset(offset)
    ...
    // 将 leftJoin 结果转换为 { ...news, category } 格式
    ...
    const [rows, total] = await Promise.all([
      db
        .select({
          ...
          ...Object.fromEntries(
            Object.entries(columnsObject).map(([key]) => [key, (newsTable as any)[key]])
          ),
          category: {
            categoryId: categoryTable.categoryId,
            categoryName: categoryTable.categoryName,
            urlKey: categoryTable.urlKey,
