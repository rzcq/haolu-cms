    ...
    <el-table v-loading="loading" :data="aiConfigList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="服务商" align="center" prop="provider" width="150">
        <template #default="scope">
          <el-tag :type="providerOptions.find(p => p.value === scope.row.provider)?.listClass || 'info'">
            {{ providerOptions.find(p => p.value === scope.row.provider)?.label || scope.row.provider }}
          </el-tag>
        </template>
      </el-table-column>
    ...
    <el-dialog title="配置详情" v-model="viewDialogOpen" width="700px" append-to-body>
      <el-descriptions :column="1" border>
        ...
        <el-descriptions-item label="服务商">
          <el-tag :type="providerOptions.find(p => p.value === currentConfig.provider)?.listClass || 'info'">
            {{ providerOptions.find(p => p.value === currentConfig.provider)?.label || currentConfig.provider }}
          </el-tag>
...
<script setup name="AiConfig">
  ...
  // 服务商列表（从字典表 ai_provider 动态加载）
  const providerOptions = ref([]);
  // 服务商默认配置模板（从字典 remark JSON 解析）
  const providerTemplates = ref({});

  /** 加载AI服务商字典数据 */
  async function loadProviderDict() {
    try {
      const res = await getDicts('ai_provider');
      if (res.data) {
        providerOptions.value = res.data.map(item => ({
          label: item.dictLabel,
          value: item.dictValue,
          listClass: item.listClass || 'info'
        }));
        // 从 remark 字段解析默认配置模板
        const templates = {};
        res.data.forEach(item => {
          if (item.remark) {
            try {
              templates[item.dictValue] = JSON.parse(item.remark);
            } catch (e) {
              console.warn('解析服务商默认配置失败:', item.dictValue, e);
            }
          }
        });
      ...
      console.error('加载AI服务商字典失败:', error);
    }
  }
  ...
  function handleProviderChange(provider) {
    const template = providerTemplates.value[provider];
    if (template) {
      form.value.endpoint = template.defaultEndpoint || '';
      form.value.model = template.defaultModel || '';
      form.value.temperature = template.temperature || 0.7;
      form.value.maxTokens = template.maxTokens || 2000;
    }
  }
