    ...
    <el-dialog :title="title" v-model="open" width="900px" append-to-body>
      <el-form v-if="open" ref="productRef" :model="form" :rules="rules" label-width="160px">
        ...
        <el-form-item label="排序">
          <el-input-number v-model="form.orderNum" :min="0" :max="9999" controls-position="right" placeholder="越小越靠前" />
          <span style="margin-left: 10px; color: #999; font-size: 12px">数字越小越靠前，留空默认 0</span>
        </el-form-item>
        ...
        <!-- 规格参数（按分类模板动态渲染） -->
        <template v-if="paramTemplate.length > 0">
          <el-divider content-position="left">
            <span style="font-size: 14px; font-weight: 600">📋 技术参数</span>
            <span style="margin-left: 8px; font-size: 12px; color: #999; font-weight: normal">
              （由分类「{{ paramTemplateSource?.categoryName?.zh || '' }}」定义）
            </span>
          </el-divider>
          <el-form-item
            v-for="param in paramTemplate"
            :key="param.key"
            :label="param.label.zh"
          >
            <el-input
              v-model="form.specifications[param.key]"
              :placeholder="`请输入${param.label.zh}`"
              style="width: 300px"
            >
...
<script setup name="Product" lang="ts">
  ...
  const paramTemplateSource = ref(null);
  ...
  function reset() {
    ...
    paramTemplate.value = [];
    paramTemplateSource.value = null;
   ...
   * 获取分类参数模板
   */
  async function fetchParamTemplate(categoryId) {
    if (!categoryId) {
      paramTemplate.value = [];
      paramTemplateSource.value = null;
      return;
    ...
    try {
      const response = await getCategoryParamTemplate(categoryId);
      paramTemplate.value = response.data.paramTemplate || [];
      paramTemplateSource.value = response.data;
      // 确保 specifications 中有模板定义的 key
      if (paramTemplate.value.length > 0 && !form.value.specifications) {
  ...
  watch(() => form.value.categoryId, (newVal) => {
    fetchParamTemplate(newVal);
  });

  // 打开新增时分类监听会在 watch 中自动触发
